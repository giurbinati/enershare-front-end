...
GBX.referenceObject = new Object3D();
GBX.triangle = new Triangle(); // used by GBX.getPlane
...
GBX.parseFile = async function( gbxml )  {
	//console.log( 'gbxml', gbxml );
	/*let url = "http://localhost:8080/api/build_geom_data/saveGBXML/";

	let xhr1 = new XMLHttpRequest()

	let formData = new FormData();
	await formData.append("test", gbxml);

	xhr1.open( 'POST', url, true );
	xhr1.setRequestHeader("Content-Type", "multipart/form-data; boundary=???");
	...
	xhr1.onerror = function( xhr ) { console.log( 'error:', xhr1  ); };
	xhr1.onprogress = function( xhr ) { FOB.onProgress( xhr1.loaded, FOB.note ); };
	xhr1.onload = function( xhr ) { FOB.onProgress( xhr1.loaded ); FOB.callbackDecider( xhr1 ); };
	let boh = formData.entries()
	xhr1.send( formData )*/
...
GBX.getSurfaceMeshes = function( surfaces ) {
	...
	GBX.materialType = THR.scene.getObjectByName( 'lightAmbient') ? MeshPhongMaterial : MeshBasicMaterial;
...
GBX.getSurfaceMesh = function( arr, index, holes ) {
	...
	const color = new Color( GBX.colors[ surfaceType ] );
	//console.log( 'color', color );

	const v = ( arr ) => new Vector3().fromArray( arr );
...
GBX.setPolygon = function( points, color, holes = [] ) {
	...
	const triangles = ShapeUtils.triangulateShape( pointsFlat, holes );
...
GBX.getPlane = function( points, start = 0 ) {
	...
	return GBX.triangle.getPlane( new Plane() );
