...
import THR from "./thr-threejs-new";
import THRR from "./thrr-three-raycaster-new";
import GBX from "./gbx-gbxml-parser-new";
...
THRU.addText = function( text = "Hello world!\n123", position = new THREE.Vector3() ) {
	...
	let textMesh = new troika_3d_text.TextMesh();
	...
	let bbox = textMesh.geometry; //.computeBoundingBox();
  ...
  THRU.toggleTitles = function() {
	...
	THR.group.children.map( child => {
		...
		let text = child.name;
		let position = child.position.clone().add( new THREE.Vector3( 0, 0, 8 ) )
...
////////// camera
  ...
  THRU.toggleCameraOrthoPerspective = function() {
	...
	if ( THRU.cameraOrtho === undefined ) {
		...
		THRU.controlsOrtho = new OrbitControls( THRU.cameraOrtho, THR.renderer.domElement );
	}
...
THRU.setCameraNear = function( range) {
	...
	let distance = THR.camera.position.distanceTo( THR.axesHelper.position ) + THR.radius;
