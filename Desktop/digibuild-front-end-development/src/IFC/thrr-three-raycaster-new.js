...
import THRU from './thru-three-utilities-new'
...
THRR.raycaster = new THREE.Raycaster();
THRR.pointer = new THREE.Vector2();
THRR.intersectObjects = [];
...
THRR.onPointerDown = function ( event ) {
	...
	if ( intersects.length ) {
		...
		if ( event.button === 2 && THRR.intersected.object.geometry.vertices ) {
			...
			let vertices = THRR.intersected.object.geometry.vertices;
			...
			let texts = vertices.map( ( vtx, i ) => THRU.drawPlacard( "" + i, THR.radius / 2000, 0xffffff, vtx.x, vtx.y, vtx.z ) );
