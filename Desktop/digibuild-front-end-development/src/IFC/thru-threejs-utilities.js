...
THRU.zoomObjectBoundingSphere = function( obj = GBX.meshGroup ) {
	...
	THR.camera.position.copy( THRU.center.clone().add( new Vector3( 1.5 * THRU.radius, 1.5 * THRU.radius, 1.5 * THRU.radius ) ) );
	...
	if ( THRU.lightDirectional ) {
		...
		THRU.lightDirectional.position.copy( THRU.center.clone().add( new Vector3( 1.5 * THRU.radius, -1.5 * THRU.radius, 1.5 * THRU.radius ) ) );
...
THRU.getMeshesVisible = function ( objThree = THR.scene ) { // not??
	...
	THRU.meshGroupVisible = new Object3D();
...
THRU.toggleSurfaceNormals = function( obj = THR.scene ) {
	...
	if ( THRU.helperNormalsFaces === undefined ) {
		...
		THRU.helperNormalsFaces = new Group();
		...
		obj.traverse( function ( child ) {
			...
			if ( child instanceof Mesh && child.visible ) {
				...
				if ( child.geometry.type === 'Geometry' ) {
					...
					const helperNormalsFace = new FaceNormalsHelper( child, 2, 0xff00ff, 3 );
					...
					const helperNormalsFace = new FaceNormalsHelper( mesh, 0.05 * THRU.radius, 0xff00ff, 3 );
...
THRU.toggleAxesHelper = function() {
	...
	if ( !THRU.axesHelper ) {
		...
		THRU.axesHelper = new AxesHelper();
...
THRU.toggleBoundingBoxHelper = function( objThree = THR.scene ){
	...
	if ( !THRU.boundingBoxHelper ) {
		...
		const bbox = new Box3().setFromObject( objThree );

		THRU.boundingBoxHelper = new Box3Helper( bbox, 0xff0000 );
...
THRU.toggleGroundHelper = function( position = THR.scene.position.clone(), elevation = 0 ) {
	...
	if ( !THRU.groundHelper ) {
		...
		const geometry = new PlaneGeometry( 2 * THRU.radius, 2 * THRU.radius);
		const material = new MeshPhongMaterial( { color: 0x888888, opacity: 0.5, side: 2 } );
		THRU.groundHelper = new Mesh( geometry, material );
...
THRU.getMeshEdges = function( obj = THR.scene ) {
	...
	for ( let mesh of obj.children ) {
		...
		const surfaceEdge = new LineSegments( edgesGeometry, lineMaterial );
...
THRU.addSomeLights = function() {
	...
	THR.renderer.shadowMap.type = PCFSoftShadowMap;

	THRU.lightAmbient = new AmbientLight( 0x888888 );
	THRU.lightAmbient.name = 'lightAmbient';
	THR.scene.add( THRU.lightAmbient );

	THRU.light1 = new DirectionalLight( 0xffffff, 0.75 );
	THRU.light1.position.set( 1, 1, 1 ).normalize();
	THRU.light1.castShadow = true;
	THR.scene.add( THRU.light1 );

	THRU.light2 = new DirectionalLight( 0xffffff, 0.5 );
...
THRU.addSomeLights2 = function() {
	...
	THRU.lightAmbient = new AmbientLight( 0x444444 );
	THRU.lightAmbient.name = 'lightAmbient';
	THR.scene.add( THRU.lightAmbient );

	THRU.lightDirectional = new DirectionalLight( 0xffffff, 1 );
...
THRU.getGeometry = function() {
	...
	const surfaceEdge = new LineSegments( edgesGeometry, edgesMaterial );
...
THRU.getSomeBoxes = function( count = 500, size = 10, material = new MeshNormalMaterial() ) {
...
THRU.drawPlacard = function( text = 'abc', scale = 0.05, color = Math.floor( Math.random() * 255 ), x = 0, y = 0, z = 10 ) {
	...
	const placard = new Object3D();

	const texture = canvasMultilineText( text, { backgroundColor: color }   );
	const spriteMaterial = new SpriteMaterial( { map: texture, opacity: 0.9, transparent: true } );
